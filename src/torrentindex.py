...
def torrent_info_matches (info_1, info_2):
	...
	# I guess if you have two torrents on the exact same data with different trackers, you lose.
	if 'files' in info_1:
...
class Torrent:
	...
	def check_data_file (self, path, length):
		...
		size_match = 1.0 - (float(abs(data_file_size - length)) / float(max(data_file_size, length)))
	...
	def add_data_path (self, config, path):
		...
		if self.type == 'single':
			...
			match_strength = self.check_data_file (path, self.file_info['length']) / 2.0
		else:
			assert (os.path.isdir (path))
			match_strength = 0.0
			for data_file_info in self.file_info['files']:
				data_file_path = os.path.join(path, '/'.join(data_file_info['path']))
				match_strength += self.check_data_file (data_file_path, data_file_info['length'])
