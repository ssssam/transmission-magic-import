...
class Search:
	...
	def search_directory_fast (self, path):
		...
		# Search subdirs now, so we can hopefully match as many torrents as possible the quick way
		...
		for filename in contents:
			if filename in self.index.names_single:
				self.match_on_name (os.path.join (path, filename), self.index.names_single[filename])
...
class cmd_search(commands.Command):
	...
	def run(self, config, help=None):
		...
		for torrent in index.torrents:
			...
			#	print "More than one location for %s: %s" % (torrent, torrent.data_matches)
